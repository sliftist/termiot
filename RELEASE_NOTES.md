...
- The scroll position is stored per tab and persisted to disk (`-1` while pinned to the live bottom, otherwise counted as lines up from the bottom), so it survives the terminal buffer being rebuilt from the log.
- When older history is loaded and prepended above a scrolled-up view, the view stays anchored on the same lines rather than sliding as the earlier output arrives.
- Scrolling is now anchored to the top visible row instead of an offset from the bottom, and scrolling all the way down automatically re-pins the view to follow live output again.
...
- On resume, the `?9001` win32-input-mode enable sequence is sent only once at startup and so lived only in old scrollback, not the live tail being parsed.
